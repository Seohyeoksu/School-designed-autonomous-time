...
총 차시: ${data.total_hours}차시, 주당 ${data.weekly_hours}차시
...
요구사항: ${data.requirements}을 가장 많이 반영해서 작성하면면 좋겠어.
학교급: ${data.school_type}도 반영해야 한다.
대상 학년: ${data.grades?.join(', ')}을 고려해서 작성해야 한다.
연계 교과: ${data.subjects?.join(', ')}
...
JSON 형식으로만 작성하고, 불필요한 문장은 쓰지 마세요.
...
function buildStep5Prompt(data: any): string {
  return `이전 단계(성취기준): ${JSON.stringify(data.standards)}
...
[이전 단계 결과]
대상 학년 ${data.grades?.join(', ')}에 맞는 수준으로 작성해야 한다.
- 영역명: ${domain}
...
1. 대상 학년: ${data.grades?.join(', ')}에 알맞은 수업계획 작성하기
...
주의: lesson_number는 "1", "2", "3"과 같이 숫자만 입력하세요.`;
}
